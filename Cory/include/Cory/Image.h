...
public:
  void create(GraphicsContext &ctx, glm::uvec3 size, uint32_t mipLevels,
  ...
  // void copy_to(graphics_context &ctx, device_buffer &rhs, vk::DeviceSize
  // size);

  /**
   * generates mipmaps for a texture. dstLayout and dstAccess specify the
   * configuration that the texture should be transitioned to after the mipmap
   * generation
   */

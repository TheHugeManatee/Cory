...
class Mesh {
  public:
        ...
        , m_topology{topology}
    {
        ...
        if constexpr (std::is_same_v<IndexType, std::uint8_t>)
            m_indexType =
                vk::IndexType::eUint8EXT; // looks like we would need an extension for that though
  ...
  private:
    void createVertexBuffer(const void *vertexData, const vk::DeviceSize dataSize);

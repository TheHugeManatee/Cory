...
public:
  ...
  Stats stats() const
  {
    ...
    auto endIter = m_currentIdx > m_data.size()
                       ? m_data.cend()
                       : m_data.cbegin() + m_currentIdx;
    auto stats = std::accumulate(++m_data.cbegin(), endIter,
    ...
    stats.avg /= (m_currentIdx > RECORD_HISTORY_SIZE) ? RECORD_HISTORY_SIZE
                                                      : m_currentIdx;
    return stats;
  }
  ...
  std::vector<long long> history() const
  {
    ...
    if (breakPoint > 0)
      std::copy(m_data.cbegin(), m_data.cbegin() + breakPoint,
...
public:
  ...
  static std::unordered_map<std::string, Record> GetRecords()
  {
    return s_records;
  }
...
public:
  ...
  LapTimer(std::chrono::milliseconds reportInterval = std::chrono::milliseconds{
               1000});

...
enum class DeviceMemoryUsage : std::underlying_type<VmaMemoryUsage>::type {
  ...
  eGpuOnly =
      VMA_MEMORY_USAGE_GPU_ONLY, ///< textures, images used as attachments
  eCpuOnly = VMA_MEMORY_USAGE_CPU_ONLY, ///< staging buffers
  eCpuToGpu =
      VMA_MEMORY_USAGE_CPU_TO_GPU, ///< dynamic resources, i.e. vertex/uniform
                                   ///< buffers, dynamic textures
  eGpuToCpu =
      VMA_MEMORY_USAGE_GPU_TO_CPU, ///< transform feedback, screenshots etc.
  eCpuCopy = VMA_MEMORY_USAGE_CPU_COPY, ///< staging custom paging/residency
  eGpuLazilyAllocated =
      VMA_MEMORY_USAGE_GPU_LAZILY_ALLOCATED ///< transient attachment images,
...
// figure out which queue families are supported (like memory transfer, compute,
// graphics etc.)
...
vk::PipelineViewportStateCreateInfo ViewportState(vk::Viewport &viewport,
                                                  vk::Rect2D &scissor);

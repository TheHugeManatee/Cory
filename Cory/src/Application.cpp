...
void Application::cursorPosCallback(GLFWwindow *window, double mouseX,
                                    double mouseY)
{
  ...
  if (mouseButton != Cory::CameraManipulator::MouseButton::None) {
    ...
    cameraManipulator.mouseMove(
        glm::ivec2(static_cast<int>(mouseX), static_cast<int>(mouseY)),
                                      ...
                                      int /*action*/, int /*mods*/)
{
  ...
  Cory::CameraManipulator &cameraManipulator =
      reinterpret_cast<Application *>(glfwGetWindowUserPointer(window))
          ->cameraManipulator;
  cameraManipulator.setMousePosition(
      glm::ivec2(static_cast<int>(xpos), static_cast<int>(ypos)));
}
...
void Application::run()
{
  ...
  m_imgui->init(window(), ctx(), msaaSamples(), colorBuffer().view(),
                swapChain());
  ...
  if (enableValidationLayers) {
    m_ctx.instance->destroyDebugUtilsMessengerEXT(m_debugMessenger, nullptr,
                                                  m_ctx.dl);
  }
}
...
{
  m_requestedExtensions.insert(end(m_requestedExtensions), begin(extensions),
                               end(extensions));
}
...
void Application::initWindow()
{
  ...
  m_window =
      glfwCreateWindow(m_initialWindowSize.width, m_initialWindowSize.height,
  ...
  cameraManipulator.setLookat(1.0f * diagonal, (sceneMin + sceneMax) / 2.f,
                              glm::vec3(0, 1, 0));
  glfwSetWindowUserPointer(m_window, this);
}
...
void Application::setupInstance()
{
  ...
  if (enableValidationLayers) {
    createInfo.enabledLayerCount =
        static_cast<uint32_t>(m_requestedLayers.size());
...
void Application::createSurface()
{
  ...
  if (glfwCreateWindowSurface(*m_ctx.instance, m_window, nullptr, &surface) !=
      VK_SUCCESS) {
...
void Application::createLogicalDevice()
{
  ...
  std::set<uint32_t> uniqueQueueFamilies = {
      m_ctx.queueFamilyIndices.graphicsFamily.value(),
  ...
  createInfo.enabledExtensionCount =
      static_cast<uint32_t>(m_requestedExtensions.size());
  createInfo.ppEnabledExtensionNames = m_requestedExtensions.data();

  // device-specific layers - these are already covered by the instance layers,
  // not strictly needed again here but seems to be good practice.
  if (enableValidationLayers) {
    createInfo.enabledLayerCount =
        static_cast<uint32_t>(m_requestedLayers.size());
    createInfo.ppEnabledLayerNames = m_requestedLayers.data();
  ...
  m_ctx.graphicsQueue = m_ctx.device->getQueue(
      m_ctx.queueFamilyIndices.graphicsFamily.value(), 0);
  m_ctx.presentQueue =
      m_ctx.device->getQueue(m_ctx.queueFamilyIndices.presentFamily.value(), 0);
}
...
void Application::createCommandPools()
{
  ...
  poolInfo.flags = vk::CommandPoolCreateFlagBits(
      0); // for re-recording of command buffers,
          // VK_COMMAND_POOL_CREATE_TRANSIENT_BIT or
          // VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT might be necessary
...
void Application::createSyncObjects(uint32_t maxFramesInFlight)
{
  ...
  for (size_t i{}; i < maxFramesInFlight; ++i) {
    m_imageAvailableSemaphores[i] =
        m_ctx.device->createSemaphoreUnique(semaphoreInfo);
...
void Application::createColorResources()
{
  m_renderTarget.create(
      m_ctx, {m_swapChain->extent().width, m_swapChain->extent().height, 1},
...
void Application::drawFrame()
{
  ...
  if (m_imagesInFlight[imageIndex] != vk::Fence{}) {
    result = m_ctx.device->waitForFences({m_imagesInFlight[imageIndex]}, true,
                                         UINT64_MAX);
...
void Application::processPerfCounters(LapTimer &fpsCounter)
{
  ...
  std::ranges::transform(
      fpsHistory, std::back_inserter(fpsHistoryMs),
      [](long long nanos) { return float(nanos) / 1'000'000; });
  ImGui::PlotLines("Frame Times", fpsHistoryMs.data(),
                   static_cast<int>(fpsHistoryMs.size()), 0, nullptr, 0.0f,
  ...
  if (ImGui::CollapsingHeader("Perf Markers")) {
    ...
    for (const auto [k, v] : recs) {
      ...
      float visStats[3]{float(ps.avg) / 1'000, float(ps.min) / 1'000,
                        float(ps.max) / 1'000};
      //             auto txt = fmt::format("{:15} {:3.2f} ({:3.2f}-{:3.2f})",
...
void Application::recreateSwapChain()
{
  ...
  m_swapChain =
      {}; // first get rid of the old swap chain before creating the new one
  m_swapChain = std::make_unique<SwapChain>(m_ctx, m_window, m_surface);
...
void Application::setupDebugMessenger()
{
  ...
  m_debugMessenger = m_ctx.instance->createDebugUtilsMessengerEXT(
      createInfo, nullptr, m_ctx.dl);
}
...
bool Application::isDeviceSuitable(const vk::PhysicalDevice &device)
{
  ...
  // CO_CORE_INFO("  {} Driver {}, API {}", deviceProperties,
  // deviceProperties.driverVersion, deviceProperties.apiVersion);

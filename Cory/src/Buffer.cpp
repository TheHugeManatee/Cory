...
void Buffer::create(GraphicsContext &ctx, vk::DeviceSize size,
                    vk::BufferUsageFlags usage, DeviceMemoryUsage memUsage)
{
  ...
  // vk::SharingMode::eExclusive; // sharing between queue families - we don't
  // do that atm
  ...
  VkBuffer buffer;
  VkResult result =
      vmaCreateBuffer(ctx.allocator, &bufferInfo, &allocCreateInfo, &buffer,
                      &m_allocation, &allocInfo);

  if (result != VK_SUCCESS) {
    throw std::runtime_error(
        "Could not allocate buffer from memory allocator!");
  }
  ...
  vk::DebugUtilsObjectNameInfoEXT debugUtilsObjectNameInfo(
      ...
      NON_DISPATCHABLE_HANDLE_TO_UINT64_CAST(VkBuffer, m_buffer), name.c_str());
...
void Buffer::upload(GraphicsContext &ctx, const void *srcData,
                    vk::DeviceSize size)
{
  ...
  // NOTE 2: CPU-GPU transfer happens in the background and is guaranteed to
  // complete before the next vkQueueSubmit()
}
...
void Buffer::copyTo(GraphicsContext &ctx, Buffer &rhs,

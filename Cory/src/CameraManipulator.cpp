...
// at
// https://github.com/KhronosGroup/Vulkan-Hpp/blob/master/samples/RayTracing/CameraManipulator.cpp
//
...
#pragma clang diagnostic ignored                                               \
    "-Wdeprecated-volatile" // to keep glm/detail/type_half.inl compiling
#endif
...
glm::mat4 const &CameraManipulator::getMatrix() const { return m_matrix; }
...
glm::ivec2 const &CameraManipulator::getWindowSize() const
{
  return m_windowSize;
}

CameraManipulator::Action
CameraManipulator::mouseMove(glm::ivec2 const &position,
                             MouseButton mouseButton, ModifierFlags &modifiers)
{
  Action curAction = Action::None;
  switch (mouseButton) {
  case MouseButton::Left:
    if (((modifiers & ModifierFlagBits::Ctrl) &&
...
{
  glm::vec2 delta(
      float(position[0] - m_mousePosition[0]) / float(m_windowSize[0]),
...
void CameraManipulator::orbit(glm::vec2 const &delta, bool invert)
{
  ...
  glm::vec4 tmpVector =
      yRotation * glm::vec4(centerToEye.x, centerToEye.y, centerToEye.z, 0.0f);
  ...
  tmpVector =
      xRotation * glm::vec4(centerToEye.x, centerToEye.y, centerToEye.z, 0);
...
{
  glm::vec2 p0(2 * (m_mousePosition[0] - m_windowSize[0] / 2.) /
                   double(m_windowSize[0]),
               2 * (m_windowSize[1] / 2. - m_mousePosition[1]) /
                   double(m_windowSize[1]));
  glm::vec2 p1(
      2 * (position[0] - m_windowSize[0] / 2.) / double(m_windowSize[0]),
      2 * (m_windowSize[1] / 2. - position[1]) / double(m_windowSize[1]));

  // correct for rounding errors
  if (glm::length(p1 - p0) < 0.0001f)
    return;
...

  {
    ...
    glm::mat4 rot_mat =
        glm::rotate(rad, glm::vec3(rot_axis.x, rot_axis.y, rot_axis.z));

    glm::vec3 pnt = m_cameraPosition - m_centerPosition;
    glm::vec4 pnt2 = rot_mat * glm::vec4(pnt.x, pnt.y, pnt.z, 1);
    glm::vec4 up2 =
        rot_mat * glm::vec4(m_upVector.x, m_upVector.y, m_upVector.z, 0);

    if (glm::isnan(glm::length(pnt)) || glm::isnan(glm::length(up2)) ||
        glm::isnan(glm::length(pnt2))) {
      CO_CORE_ERROR("NaN dected in camera.");
      return;
    }

...
SwapChain::SwapChain(GraphicsContext &ctx, GLFWwindow *window,
...
void SwapChain::createSwapchain(vk::SurfaceKHR surface)
{
  ...
  // we use one more image as a buffer to avoid stalls when waiting for the next
  ...
  // this might be 2 if we are developing stereoscopic stuff
  createInfo.imageArrayLayers = 1;

  // for off-screen rendering, it is possible to use
  ...
  // be shareable
  if (m_ctx.queueFamilyIndices.graphicsFamily !=
      m_ctx.queueFamilyIndices.presentFamily) {
    createInfo.imageSharingMode = vk::SharingMode::eConcurrent;
    createInfo.queueFamilyIndexCount = 2;
    createInfo.pQueueFamilyIndices = queueFamilyIndices;
  }
  else {
    // exclusive has better performance
  ...
  // whether the alpha channel should be used to composite on top of other
  // windows
  createInfo.compositeAlpha = vk::CompositeAlphaFlagBitsKHR::eOpaque;

  createInfo.presentMode = presentMode;

  // false would force pixels to be rendered even if they are
  // occluded. might be important if the buffer is read back
  // somehow (screen shots etc?)
  createInfo.clipped = VK_TRUE;

  // old swap chain, required when resizing etc.
...
{
  // for high DPI, the extent between pixel size and screen coordinates might
  // not be the same. in this case, we need to compute a proper viewport extent
  // from the GLFW screen coordinates
...
{
  // BRGA8 and SRGB are the preferred formats, otherwise fall back to any
  // available one

...
void Texture::create(GraphicsContext &ctx, glm::uvec3 size, uint32_t mipLevels,
                     vk::ImageType type, vk::Format format,
                     vk::ImageTiling tiling, vk::Filter filter,
                     ...
                     vk::ImageUsageFlags usage, DeviceMemoryUsage memoryUsage)
{
  ...
  VkResult result = vmaCreateImage(
      ctx.allocator, (VkImageCreateInfo *)&imageInfo, &allocCreateInfo,
  ...
  vk::DebugUtilsObjectNameInfoEXT debugUtilsObjectNameInfo(
      ...
      NON_DISPATCHABLE_HANDLE_TO_UINT64_CAST(VkImage, m_image), m_name.c_str());
  ctx.device->setDebugUtilsObjectNameEXT(debugUtilsObjectNameInfo, ctx.dl);
#endif
}

void Texture::upload(GraphicsContext &ctx, const void *srcData,
                     vk::DeviceSize size)
{
  void *mappedData;
                              ...
                              vk::AccessFlags dstAccess)
{
  ...
  vk::FormatProperties formatProperties =
      ctx.physicalDevice.getFormatProperties(m_format);
  ...
  for (uint32_t i = 1; i < m_mipLevels; ++i) {
    ...
    blit.dstOffsets[1] = vk::Offset3D{mipSize.x > 1 ? mipSize.x / 2 : 1,
                                      mipSize.y > 1 ? mipSize.y / 2 : 1, 1};
...
void DepthBuffer::create(GraphicsContext &ctx, glm::uvec3 size,
                         vk::Format format, vk::SampleCountFlagBits msaaSamples)
{
  m_size = size;
  m_mipLevels = 1;
  m_format = format;
...
void RenderBuffer::create(GraphicsContext &ctx, glm::uvec3 size,
                          vk::Format format,

...
PipelineBuilder &PipelineBuilder::setShaders(std::vector<Shader> shaders)
{
  ...
  // TODO: push constants here? Note: pSpecializationInfo can be used to set
  // compile time constants - kinda like macros in an online compilation?
  return *this;
}

Cory::PipelineBuilder &PipelineBuilder::setVertexInput(const Mesh &mesh)
{
  return setVertexInput(mesh.bindingDescription(), mesh.attributeDescriptions(),
                        mesh.topology());
}
...
RenderPassBuilder::addDepthAttachment(vk::Format format,
                                      vk::SampleCountFlagBits samples)
{
  assert(!m_depthStencilAttachmentRef.has_value() &&
  ...
  m_depthStencilAttachmentRef = addAttachment(
      depthAttachment, vk::ImageLayout::eDepthStencilAttachmentOptimal);
  return *m_depthStencilAttachmentRef;
}

vk::AttachmentReference
RenderPassBuilder::addResolveAttachment(vk::Format format,
                                        vk::ImageLayout finalLayout)
{
  ...
  auto attachmentRef = addAttachment(colorAttachmentResolve,
...
uint32_t RenderPassBuilder::addDefaultSubpass()
{
  ...
  subpass.colorAttachmentCount =
      static_cast<uint32_t>(m_colorAttachmentRefs.size());

...
void HelloTriangleApplication::createCommandBuffers()
{
  ...
  allocInfo.level =
      vk::CommandBufferLevel::ePrimary; // _SECONDARY cannot be directly
                                        // submitted but can be called from
                                        // other cmd buffer
...
void HelloTriangleApplication::updateUniformBuffer(uint32_t imageIndex)
{
  ...
  ubo.proj = glm::perspective(glm::radians(45.0f),
                              ...
                              0.1f, 10.0f);
  ubo.proj[1][1] *=
      -1; // NOTE: we flip this bc/ glm is written for OpenGL which has Y
...
Texture
HelloTriangleApplication::createTextureImage(std::string textureFilename,
                                             vk::Filter filter,
